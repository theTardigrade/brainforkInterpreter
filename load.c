...
static void readFile(FILE *fp, char **contents)
{
	...
	while ( contentsSize < (size_t) globalOptions.maxFileSize && (*(*contents + contentsSize++) = fgetc(fp)) != EOF ) 
		if ( contentsSize >= contentsMemSize ) {
			contentsMemSize = MIN_OF_THREE(contentsMemSize * 2, fileSize, (size_t) globalOptions.maxFileSize); /* limited exponential allocation */
